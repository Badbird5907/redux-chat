...
export const { GET, POST } = createServerFn({ method: 'POST' })
  .inputValidator(requestBody)
  .handler(async ({ request, input }) => {
    const { threadId, userMessageId, id } = input;
    ...
    return result.toUIMessageStreamResponse({
      ...
      consumeSseStream: async ({ stream }) => {
        ...
        const streamContext = createResumableStreamContext({
          // Note: In TanStack Start, we'll need to handle this differently
          // For now, we'll return the stream directly
        });
