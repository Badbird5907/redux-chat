...
function buildRedemptionColumns(handlers: {
  revoke: (redemptionId: string) => void;
  revokeStripeInvoiceCredit: (redemptionId: string) => void;
}): ColumnDef<PromotionRedemptionRow>[] {
  return [
    {
      accessorKey: "userId",
      header: "User",
          ...
          <div className="text-right">
            {redemption.status === "applied" && redemption.appCreditGrantId ? (
              <Button
                ...
                size="sm"
                variant="outline"
                onClick={() => void handlers.revoke(redemption.redemptionId)}
              >
                Revoke
              </Button>
              ...
              <Button
                ...
                variant="outline"
                onClick={() =>
                  void handlers.revokeStripeInvoiceCredit(
  ...
  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2">
        <Button type="button" variant="secondary" onClick={onCopyUrl}>
...
export function PromotionStatsCard({
  promotion,
  usageSummary,
  remaining,
}: {
  promotion: {
    perUserRedemptionLabel: string;
...
export function RedemptionFiltersBar({
  statusFilter,
...
export function AdminPromotionDetailPage() {
  ...
  const columns = buildRedemptionColumns({
    ...
    revokeStripeInvoiceCredit: (id) => void revokeStripeInvoiceCredit(id),
  });
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
      <div className="flex flex-col gap-4">
        ...
        <PromotionDetailHeader
          promotion={promotion}
          editDialog={<PromotionFormDialog mode="edit" promotion={promotion} />}
          onCopyUrl={() => void copyRedeemUrl()}
          onPause={() => void pause()}
